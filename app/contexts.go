...
// ListRowContext provides the row list action context.
type ListRowContext struct {
	context.Context
	*goa.ResponseData
	*goa.RequestData
}

// NewListRowContext parses the incoming request URL and body, performs validations and creates the
// context used by the row controller list action.
func NewListRowContext(ctx context.Context, service *goa.Service) (*ListRowContext, error) {
	var err error
	resp := goa.ContextResponse(ctx)
	resp.Service = service
	req := goa.ContextRequest(ctx)
	rctx := ListRowContext{Context: ctx, ResponseData: resp, RequestData: req}
	return &rctx, err
}

// OK sends a HTTP response with status code 200.
func (ctx *ListRowContext) OK(r GwentapiRowCollection) error {
	ctx.ResponseData.Header().Set("Content-Type", "application/vnd.gwentapi.row+json; type=collection")
	return ctx.ResponseData.Service.Send(ctx.Context, 200, r)
}

// OKNoModel sends a HTTP response with status code 200.
func (ctx *ListRowContext) OKNoModel(r GwentapiRowNoModelCollection) error {
	ctx.ResponseData.Header().Set("Content-Type", "application/vnd.gwentapi.row+json; type=collection")
	return ctx.ResponseData.Service.Send(ctx.Context, 200, r)
}

// NotFound sends a HTTP response with status code 404.
func (ctx *ListRowContext) NotFound() error {
	ctx.ResponseData.WriteHeader(404)
	return nil
}

// InternalServerError sends a HTTP response with status code 500.
func (ctx *ListRowContext) InternalServerError() error {
	ctx.ResponseData.WriteHeader(500)
	return nil
}

// ShowRowContext provides the row show action context.
type ShowRowContext struct {
	context.Context
	*goa.ResponseData
	*goa.RequestData
	RowID string
...
// NewShowRowContext parses the incoming request URL and body, performs validations and creates the
// context used by the row controller show action.
func NewShowRowContext(ctx context.Context, service *goa.Service) (*ShowRowContext, error) {
	var err error
	resp := goa.ContextResponse(ctx)
	resp.Service = service
	req := goa.ContextRequest(ctx)
	rctx := ShowRowContext{Context: ctx, ResponseData: resp, RequestData: req}
	paramRowID := req.Params["rowID"]
	if len(paramRowID) > 0 {
		rawRowID := paramRowID[0]
		rctx.RowID = rawRowID
	}
	return &rctx, err
}
...
// OK sends a HTTP response with status code 200.
func (ctx *ShowRowContext) OK(r *GwentapiRow) error {
	ctx.ResponseData.Header().Set("Content-Type", "application/vnd.gwentapi.row+json")
	return ctx.ResponseData.Service.Send(ctx.Context, 200, r)
}

// OKNoModel sends a HTTP response with status code 200.
func (ctx *ShowRowContext) OKNoModel(r *GwentapiRowNoModel) error {
	ctx.ResponseData.Header().Set("Content-Type", "application/vnd.gwentapi.row+json")
	return ctx.ResponseData.Service.Send(ctx.Context, 200, r)
}

// NotFound sends a HTTP response with status code 404.
func (ctx *ShowRowContext) NotFound() error {
	ctx.ResponseData.WriteHeader(404)
	return nil
}

// InternalServerError sends a HTTP response with status code 500.
func (ctx *ShowRowContext) InternalServerError() error {
	ctx.ResponseData.WriteHeader(500)
	return nil
}
